...
_Which attraction had exactly 565,772 visitors in 2022?_ Knowlsey Safari and Knowsley Hall
...
_How many attraction had more than 1 million visitors in 2022?_ 22
...
_How many regions have more than 50 tourist attraction in the data set? (Hint: You will need to tabulate the data before filtering.)_ Scotland
...
The 'NA' stands for 'not available', this is happening because of missing data values, or incompatible entries in the data set that we are manipulating.
...
If the mean statistics are larger than the median statistics, this implies a positive skew exists in the data set. As there are more higher data values than there are lower ones, making the mean higher than the median value.
...
```{r Ex_n}
visitors_with_nations %>%
  mutate(pct.chg=(n_2022-n_2021)/n_2021) %>%
  filter(region == 'Scotland') %>%
  group_by(admission) %>%
  summarise(mean_pct.chg = mean(pct.chg, na.rm = TRUE))
